...
/**
 * Per-note placement picker: tick the curriculum nodes — topics and/or subtopics, across any of the
 * educator's classes — where this note should appear. Submitting reconciles the note's placements to
 * the ticked set. Mirrors the video assign dialog.
 */
export function NoteAssignDialog({ resourceId, noteTitle, currentParents, tree }: NoteAssignDialogProps) {
  ...
  const handleSubmit = (event: React.FormEvent) => {
    ...
    startTransition(async () => {
      const result = await setNotePlacementsAction(resourceId, parents);
      if (result?.error) {
      ...
      <div className="w-full max-w-lg max-h-[90vh] flex flex-col rounded-lg border border-border bg-card shadow-lg">
        <div className="flex items-start justify-between gap-4 p-6 pb-4 border-b border-border">
          <div className="min-w-0">
            ...
            <p className="text-sm text-muted-foreground truncate mt-1">{noteTitle}</p>
          </div>
          <button
            type="button"
